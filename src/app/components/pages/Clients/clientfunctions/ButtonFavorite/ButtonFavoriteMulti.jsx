...
class ButtonFavorite extends Component {
    ...
    addRemoveToMyFavorites = (id, myfave, ownerID, isFavorite, clientContactDetailsID) => {
        this.props.addRemoveToMyFavorites(id, myfave, ownerID, isFavorite, clientContactDetailsID);
    };
    ...
    selectAddress = () => {
      ...
      const clientContactDetailsID = myfave ? myfave.clientContactDetailsID : this.state.selectedAddress.id;
      const isFavorite = this.state.selectedAddress.isFavorite = !this.state.selectedAddress.isFavorite;
    ...
    render() {
      const { client, ownerID, addRemoveToMyFavorites, clientContactDetails, selectedAddress } = this.state;
      ...
      return (
          <>
                    ...
                    <div className="modal-content">
                      ...
                      <tbody>
                        {clientContactDetails.map((contact, index) => (
                          ...
                          >
                            <td>
                                <i data-toggle="tooltip"
                                title={`${
                                    contact.isFavorite ? "Remove from My Favorites." : "Add to My Favorites"
                                    }`}
                                className={`fas fa-heart ${
                                    contact.isFavorite ? "text-danger" : "text-muted"
                                }`}
                                ></i> {index + 1}.</td>
                            <td>
                        ...
                        <div className="modal-footer">
                            {selectedAddress ? selectedAddress.isFavorite ?
                                <button
                                    type="button"
                                ...
                                >
                                    <i className="fas fa-trash"></i> Remove from My Favorites
                                </button>
                                :
                                <button
                                    type="button"
                                    ...
                                    onClick={this.selectAddress}
                                >
                                    <i className="fas fa-plus-circle"></i> Add to My Favorites
                                </button>
                            : null }
                        {/* // <button
                        //     type="button"
            ...
            (<button
                className="btn btn-link p-0 float-right"
                onClick={() =>
                  this.addRemoveToMyFavorites(client.id,
                    client.myfave,
                    ownerID,
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    addRemoveToMyFavorites(clientID, myfave, ownerID, isFavorite, clientContactDetailsID) {
      ...
      if (id) dispatch(removeFromMyFavorites(id, clientContactDetailsID));
