...
export const UserEdit = ({
  ...
  return (
    <>
      {isAdmin ? (
        <>
          <ConnectedContentHeader pagename={`${firstName} ${lastName}`} />
                        ...
                        <div className="form-row">
                          <div className="form-group col-md-6">
                            <ConnectedInputForm
                              ...
                              required
                              isDisabled={id === "User1" ? true : false}
                            />
                          </div>
                          <div className="form-group col-md-6">
                            <ConnectedInputForm
...
const mapStateToProps = (state, ownProps) => {
  ...
  const userdetails = getUserDetails(isAdmin);
  ...
  function getUserDetails(isAdmin) {
    if (isAdmin) {
      let newUserdetails = JSON.parse(
        JSON.stringify(state.users.find((user) => user.id === id))
      );
      newUserdetails.location = newUserdetails.location.map((loc) => {
        return { value: loc, label: loc };
      });
      newUserdetails.selectedlocation = [];
